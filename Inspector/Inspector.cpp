...
Inspector::Inspector(EventLoop* loop,
    ...
    : server_(loop, httpAddr, "Inspector:"+name),
      ...
      performanceInspector_(new PerformanceInspector),
      cpuInspector_(new CpuUtilizationInspector),
      bandwidthInspector_(new BandwidthInspector)
{
    assert(CurrentThread::isMainThread());
    assert(g_globalInspector == 0);
    assert(g_InspectorPid > 0);
    ...
    cpuInspector_->registerCommands(this);
    bandwidthInspector_->registerCommands(this);
...
{
    server_.start();
    cpuInspector_->getCpuStatus();
    bandwidthInspector_->getNetStatus();
}
...
void Inspector::onRequest(const HttpRequest& req, HttpResponse& resp)
{
  ...
  {
    std::vector<string> result = split(req.getPath());
